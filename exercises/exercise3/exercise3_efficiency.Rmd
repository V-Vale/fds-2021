 ...
 * `tibble`s habe better properties when inspecting e.g. the `head` of a dataset, as it will not print as many lines as a `data.frame`. 
 * `tibble`s are more restrictive than `data.frame`s. As such, `tibble`s  will throw an error if you try to access a variable that doesn't exist in the dataset, while `data.frame`s will match names.
