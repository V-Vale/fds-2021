...
#toInstall <- c("httr","jsonlite", "yaml","guardianapi","maps","dplyr","ggplot2","digest","ROAuth","rtweet")
...
save(articles_get, file=paste("data/guardianapi-UKnews-notbrexit-",Sys.Date(),".Rda",sep=""))
```
...
```{r,echo=TRUE, eval=TRUE}
gres <- guardianapi::gu_content(query = c("UK"),
                        from_date = "2021-06-16",
                        to_date = "2021-06-17")
save(gres, file= paste("data/guardianapi-GuardianR-UKnews-2021-06-16--2021-06-17--date-",Sys.Date(),".Rda", sep=""))
```
...
friends_json <- my_oauth$OAuthRequest(URL=url, 
                                      ...
                                      method="GET")
save(friends_json, file=paste("data/guardian-friends-json-",Sys.Date(),".Rda", sep=""))
```
...
followers1_json <- my_oauth$OAuthRequest(URL=url, 
                                      params=list(screen_name = "guardian", stringify_ids="true"),
                                      method="GET")
save(followers1_json, file=paste("data/guardian-followers-1-json-",Sys.Date(),".Rda", sep=""))
```
...
followers2_json <- my_oauth$OAuthRequest(URL=url, 
                                      ...
                                      method="GET")
save(followers2_json, file=paste("data/guardian-followers-2-json-",Sys.Date(),".Rda", sep=""))
```
...
save(accounts, file=paste("data/retweeters-1254605442124255234-",Sys.Date(),".Rda",sep=""))
```
...
head(info)
save(info, file=paste("data/retweeters-1254605442124255234-",Sys.Date(),"-anonymized.Rda",sep=""))
```
